...
const getPresetItemIndex = (audioPlaylist, playlistItem) =>
  audioPlaylist.reduce(
    (result, next, x) =>
      result !== null ? result : playlistItem.id === next.id ? x : null,
    null
  )
...
class PlaylistEditor extends Component {
  ...
  updateActivePlaylistIndex = (playlistIndex) => {
    if (
      ...
      this.props.activePlaylistIndex === playlistIndex
    ) { return }
  ...
  onReorder = (newOrder) => {
    ...
    const newAudioPlaylist = newOrder.map(key =>
      audioPlaylist.find(item => `${item.key}` === key))
    const activeItemKey = this.props.audioPlaylist[
      this.props.activePlaylistIndex
    ].id
    const newActivePlaylistIndex = newAudioPlaylist.reduce(
      (answer, item, i) => (activeItemKey === item.id ? i : answer),
      0
    )
  ...
  onDelete = curry((playlistItem, e) => {
    ...
    if (activePlaylistIndex >= i) {
      const newActivePlaylistIndex = confineToRange(
        0,
        newAudioPlaylist.length - 1,
  ...
  onLockTrack = curry((playlistItem, e) => {
    ...
    const { audioPlaylist } = this.props
    const newAudioPlaylist = updateItemInPlaylist(
      audioPlaylist,
      playlistItem,
      'isLocked',
      !playlistItem.isLocked
    )
    this.props.actions.updateAudioPlaylist(newAudioPlaylist)
  })
  ...
  render() {
    ...
    const listItems = audioPlaylist.map((item, i) => {
      ...
      return {
        ...
        className: `${isActive ? 'is-active' : ''} ${
          isPlaying || item.isLocked ? 'functionality-is-disabled' : ''
        }`
      }
    })
